...
function isNumeric(textBox, name) {
	if (isNaN(textBox.value)) {
		alert("Error! " + name + " should be a number.");
		return false;
	}
	return true;
}

// Checks that number in text box is non-negative.
function isNonnegative(textBox, name) {
	if (new Number(textBox.value) < 0) {
		alert("Error! " + name + " should be non-negative.");
		return false;
	}
	return true;
}
...
function matchesPattern(textBox, name, pattern) {
	if (textBox.value.search(pattern) !=0 ) {
		alert("Error! " + name + " should be properly formatted.");
		return false;
	}
	return true;
}
